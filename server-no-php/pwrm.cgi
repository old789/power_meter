...
foreach $pair (@pairs){
  ...
  if ( $name =~ /^\D(\d+).+$/ ) {
    @payload=split(',', $value);
    if ( $1 == 0 ) $start_tick=$payload[6];
    elsif ( $payload[6] > $end_tick ) $end_tick=$payload[6];
    push(@names,$name);
}
...
my @out=();
  ...
  if ( $key =~ /^\D\d+.+$/ ) {
    ...
    @out=splice(@payload,6);
    $wr_str=POSIX::strftime("%d-%m-%Y %H:%M:%S",localtime($t)).' '.$t.' '.join(' ',$out);
  }else{
